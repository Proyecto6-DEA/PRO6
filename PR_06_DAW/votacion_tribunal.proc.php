...
$selectPregunta = $databaseConnection->prepare('SELECT * FROM tbl_notasprofe WHERE pro_id =:pro_id AND alu_id=:alu_id AND preg_id=:preg_id');
								
								$selectPregunta->bindParam(':alu_id', $alu_id);
								$selectPregunta->bindParam(':pro_id', $proId);
								$selectPregunta->bindParam(':preg_id', $preg_id);
...
$updatePregunta = $databaseConnection->prepare('UPDATE tbl_notasprofe SET np_nota = :np_nota WHERE pro_id =:pro_id AND alu_id=:alu_id AND preg_id=:preg_id');
								
								$updatePregunta->bindParam(':alu_id', $alu_id);
								$updatePregunta->bindParam(':pro_id', $proId);
								$updatePregunta->bindParam(':np_nota', $np_nota);
								$updatePregunta->bindParam(':preg_id', $preg_id);
...
$selectPreguntaCont = $databaseConnection->prepare('SELECT Avg(np_nota) FROM tbl_notasprofe, tbl_pregunta WHERE tbl_pregunta.preg_id = tbl_notasprofe.preg_id AND pro_id =:pro_id AND tbl_pregunta.preg_tipo=\'Contingut\'');
...
foreach($_REQUEST as $nombre_campo => $valor){
	...
	//echo " || ID PREGUNTA: -> ";
		...
		$resultado = $selectPregunta->fetchAll();
		//echo " VALOR DE RESTULADO: ";
		// print_r($resultado);
		...
		if(!empty($array[1])){
			...
			if(empty($resultado)){
				 //  	echo " insert: ";
					// echo " nota: ". $np_nota;
					// echo " alu id: ".$alu_id;
					// echo " proyecto id ".$proId;
					// echo " pregunta id ".$preg_id;
				$insertPregunta->execute();
			}else{
				 // 	echo "update";
					// echo "nota: ". $np_nota;
...
if (!empty($_REQUEST['id0'])) {
	...
	$SelectNotaslAlu->execute();
			// echo "nota media 1: ";
			$resultado = $SelectNotaslAlu->fetchColumn();
			$np_nota=$resultado;
