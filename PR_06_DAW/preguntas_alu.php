...
<?php  
    ...
    if (mysqli_num_rows($proyectos)>0){
        while ($proyecto=mysqli_fetch_array($proyectos)) {
           echo  $nomproy = $proyecto['proy_nom'];
           echo  $proyid = $proyecto['proy_id'];
?>
<header>;
        <div style='width:100%; height: 100%;background-color: #ffcc00'>
            <div class='centrar'>
                <div class='container'>
                    <span style='font-size: 20px'><?php echo $nomproy; ?></span>
                </div>       
            </div>             
        </div>
</header>
...
<?php
    $sql = "SELECT * FROM tbl_pregunta WHERE preg_tipo='Presentació Oral' AND preg_categoria='Alumne'";
...
echo "<div style='width:100%; height: 7%;border-bottom:  2px solid #ffcc00;background-color:white'>";
    ...
    $sql = "SELECT * FROM tbl_pregunta WHERE preg_tipo='Contingut' AND preg_categoria='Alumne'";
    $preuntas = mysqli_query($conexion, $sql);
    if (mysqli_num_rows($preuntas)>0){
    while ($preg=mysqli_fetch_array($preuntas)) {
